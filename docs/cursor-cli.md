...
### Repair
- `lumiverb repair --library <name> [--job-type embed|vision|faces|video-scenes|scene-vision|search-sync|all] [--dry-run] [--concurrency N]` — Detect and repair missing pipeline outputs. `embed`: backfill missing CLIP embeddings. `vision`: backfill missing AI descriptions. `faces`: detect faces using InsightFace (stores bounding boxes + ArcFace embeddings). `video-scenes`: run scene detection on videos with `video_indexed=false` (requires local source access). `scene-vision`: extract rep frames and run vision AI on scenes without descriptions (requires local source + vision API). `search-sync`: push stale assets to Quickwit. `all` (default): run all repair types. `--dry-run` shows what would be done without executing.
