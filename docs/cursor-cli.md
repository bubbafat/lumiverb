...
### Repair
- `lumiverb repair --library <name> [--job-type embed|vision|search-sync|all] [--dry-run] [--concurrency N]` — Detect and repair missing pipeline outputs. `embed`: backfill missing CLIP embeddings. `vision`: backfill missing AI descriptions. `search-sync`: push stale assets to Quickwit. `all` (default): run all repair types. `--dry-run` shows what would be done without executing.
