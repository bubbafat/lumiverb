...
- `worker proxy` no longer requires `tenant_id` or `storage` directly — these are encapsulated in `ArtifactStore`. In remote mode, proxy and thumbnail bytes are uploaded via `POST /v1/assets/{id}/artifacts/{type}`; keys and sha256 come from the upload response.
- `worker video-preview` no longer requires `tenant_id` or `storage` directly. It always renders preview MP4 output to a temp file, then writes bytes through `ArtifactStore` (`video_preview` type), which keeps local and remote modes aligned while preventing direct server `DATA_DIR` writes in remote mode.
- `worker video-index` writes thumbnails and scene representative frames via `ArtifactStore` (types `thumbnail` and `scene_rep`) instead of writing to `DATA_DIR` directly. In remote mode this uploads frames through the artifact API and reuses returned keys/sha256 in chunk-complete payloads.
- `worker video-vision` reads scene representative frame bytes via `ArtifactStore` (`scene_rep` with `rep_frame_ms`) and uses a per-scene temp file only for provider input. It does not read frame files directly from `DATA_DIR`.
