...
- `lumiverb worker embed [--library <name>] [--once] [--output human|jsonl]` — Run the embedding worker (CLIP + Moondream vectors for similarity search). Use `-l <name>` to scope to one library; `--once` processes the queue until empty then exits.
