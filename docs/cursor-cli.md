...
- `lumiverb upgrade [--dry-run] [--max-steps N] [--step <step_id>] [--force]` — Run tenant-level upgrade steps (schema/backfills) idempotently. With `--dry-run`, lists pending steps without executing. With `--step`, run only a single step; by default it refuses to run if preceding steps are not complete unless `--force` is provided (with a confirmation prompt).
