...
- **GET /v1/libraries/health** — Bulk health indicator. Returns `[{ "library_id", "healthy", "pending" }]` for every non-trashed library, in a single SQL call. `pending` is the number of assets that would show up on `repair --dry-run` (missing proxy / vision / embeddings / faces / face embeddings / OCR / video scenes / scene vision / transcription). `healthy` is `pending == 0`. Used by the libraries page to render a green/orange dot per library without N+1ing `/v1/assets/repair-summary`.
