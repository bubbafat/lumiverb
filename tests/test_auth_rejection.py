...
@pytest.mark.slow
def test_scans_create_no_auth(auth_rejection_client: TestClient) -> None:
    """POST /v1/scans without auth returns 401."""
    r = auth_rejection_client.post(
        "/v1/scans",
        json={"library_id": "lib_foo", "status": "running"},
    )
    assert r.status_code == 401
