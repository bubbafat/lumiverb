...
FIX-9 (RISK-2): SearchSyncQueue.enqueue() idempotent (partial unique index)
...
@pytest.mark.slow
def test_library_trash_cancels_search_sync_queue(
    audit_fixes_client: tuple[TestClient, str, str, str],
) -> None:
    """DELETE /libraries/{id} sets search_sync_queue rows to 'cancelled' for pending/processing rows."""
    client, api_key, library_id, tenant_url = audit_fixes_client
    auth = {"Authorization": f"Bearer {api_key}"}

    # Create a dedicated library with one asset
    r_lib = client.post(
        "/v1/libraries",
        json={"name": "TrashSyncQueueLib", "root_path": "/trash-ssq"},
        headers=auth,
    )
    assert r_lib.status_code == 200
    test_library_id = r_lib.json()["library_id"]

    asset_id = _upsert_asset(client, auth, test_library_id, "ssq_asset.jpg")

    # Directly insert a pending search_sync_queue row
    engine = create_engine(tenant_url)
    sync_id = "ssq_test_audit_001"
    try:
        with engine.connect() as conn:
            conn.execute(
                text(
                    "INSERT INTO search_sync_queue (sync_id, asset_id, operation, status, created_at) "
                    "VALUES (:sync_id, :asset_id, 'upsert', 'pending', NOW())"
                ),
    ...
    # The queue row should now be 'cancelled'
    engine = create_engine(tenant_url)
    try:
        with engine.connect() as conn:
            row = conn.execute(
                text("SELECT status FROM search_sync_queue WHERE sync_id = :sync_id"),
                {"sync_id": sync_id},
            ).fetchone()
            assert row is not None
            assert row[0] == "cancelled", (
                f"Expected search_sync_queue row to be 'cancelled' after library trash, got {row[0]!r}"
            )
    finally:
        engine.dispose()
...
# ---------------------------------------------------------------------------
# FIX-9 (RISK-2): SearchSyncQueue.enqueue() idempotent
# ---------------------------------------------------------------------------


@pytest.mark.slow
def test_search_sync_ingest_failure_resets_to_pending(
    audit_fixes_client: tuple[TestClient, str, str, str],
) -> None:
    """SearchSyncQueueRepository.reset_to_pending() resets processing rows to pending."""
    client, api_key, library_id, tenant_url = audit_fixes_client
    auth = {"Authorization": f"Bearer {api_key}"}

    asset_id = _upsert_asset(client, auth, library_id, "ssq_reset_pending.jpg")

    engine = create_engine(tenant_url)
    sync_id = "ssq_test_reset_001"
    try:
        with engine.connect() as conn:
            conn.execute(
                text(
                    "INSERT INTO search_sync_queue "
                    "(sync_id, asset_id, operation, status, created_at, processing_started_at) "
                    "VALUES (:sync_id, :asset_id, 'upsert', 'processing', NOW(), NOW())"
                ),
                {"sync_id": sync_id, "asset_id": asset_id},
            )
            conn.commit()
    ...
    try:
        with SQLModelSession(engine) as session:
            ...
            count = queue_repo.reset_to_pending([sync_id])
            assert count == 1, f"Expected 1 row reset, got {count}"

        with engine.connect() as conn:
            row = conn.execute(
                text(
                    "SELECT status, processing_started_at FROM search_sync_queue "
                    "WHERE sync_id = :sync_id"
                ),
                {"sync_id": sync_id},
            ).fetchone()
            assert row is not None
            assert row[0] == "pending", f"Expected status=pending, got {row[0]!r}"
            assert row[1] is None, f"Expected processing_started_at=NULL, got {row[1]!r}"
    finally:
        engine.dispose()


@pytest.mark.slow
def test_search_sync_enqueue_idempotent_concurrent(
    audit_fixes_client: tuple[TestClient, str, str, str],
) -> None:
    """Calling enqueue() twice for same (asset_id, operation) results in only one pending row."""
    client, api_key, library_id, tenant_url = audit_fixes_client
    auth = {"Authorization": f"Bearer {api_key}"}

    asset_id = _upsert_asset(client, auth, library_id, "ssq_idempotent.jpg")
    ...
    try:
        with SQLModelSession(engine) as session:
            queue_repo = SearchSyncQueueRepository(session)
            row1 = queue_repo.enqueue(asset_id=asset_id, operation="upsert")
            row2 = queue_repo.enqueue(asset_id=asset_id, operation="upsert")

        # First call should return a row; second should return None (conflict)
        assert row1 is not None, "First enqueue should return the inserted row"
        assert row2 is None, "Second enqueue should return None (already pending)"

        with engine.connect() as conn:
            count_row = conn.execute(
                text(
                    "SELECT COUNT(*)::int FROM search_sync_queue "
                    "WHERE asset_id = :asset_id AND status IN ('pending', 'processing')"
                ),
                {"asset_id": asset_id},
            ).fetchone()
            assert count_row is not None
            assert count_row[0] == 1, (
                f"Expected exactly 1 pending row after two enqueues, got {count_row[0]}"
            )
    finally:
        engine.dispose()
...
@pytest.mark.slow
def test_video_vision_complete_no_asset_sync_enqueued(
    audit_fixes_client: tuple[TestClient, str, str, str],
) -> None:
    """Completing a video-vision job does NOT enqueue an asset-level (scene_id=NULL) search_sync row."""
    client, api_key, library_id, tenant_url = audit_fixes_client
    auth = {"Authorization": f"Bearer {api_key}"}
    ...
    r_claim = client.get(
        "/v1/jobs/next",
        params={"job_type": "video-vision"},
        headers=auth,
    )
    assert r_claim.status_code == 200, (r_claim.status_code, r_claim.text)
    claimed_job_id = r_claim.json()["job_id"]

    # Clear any existing search_sync_queue rows before completing the job
    engine = create_engine(tenant_url)
    try:
        with engine.connect() as conn:
            conn.execute(
                text("DELETE FROM search_sync_queue WHERE asset_id = :asset_id"),
                {"asset_id": asset_id},
            )
            conn.commit()
    finally:
    ...
    # Complete the video-vision job
    r_complete = client.post(
        f"/v1/jobs/{claimed_job_id}/complete",
        json={},
        headers=auth,
    )
    assert r_complete.status_code == 200, (r_complete.status_code, r_complete.text)

    # Assert NO asset-level (scene_id=NULL) search_sync row was enqueued
    engine = create_engine(tenant_url)
    try:
        with engine.connect() as conn:
            row = conn.execute(
                text(
                    "SELECT COUNT(*)::int FROM search_sync_queue "
                    "WHERE asset_id = :asset_id AND scene_id IS NULL"
                ),
                {"asset_id": asset_id},
            ).fetchone()
            assert row is not None
            assert row[0] == 0, (
                f"Expected 0 asset-level search_sync rows after video-vision complete, got {row[0]}"
            )
    finally:
        engine.dispose()
