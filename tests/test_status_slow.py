"""Slow tests for status commands and pipeline_status repository."""
...
def _ensure_library_asset(session: Session, library_id: str, asset_id: str, rel_path: str) -> None:
    ...
    session.execute(
        text(
            ...
            VALUES (:asset_id, :lib_id, :rel_path, 0, 'image/jpeg', 'online', 'proxied', NOW(), NOW())
...
def test_pipeline_status_latest_state_only(tenant_db_session: Session) -> None:
    ...
    pipeline_status must count latest state only: completed=1, failed=0.
    """
    ...
    # Use explicit created_at so the last (completed) is the most recent.
    jobs = [
